在训练模型中，很可能希望一个网络的不同模块具有不同的学习率，比如backbone的学习率为0.0001，而全连接层的学习率为0.0001。为实现该功能，需要了解pytorch[优化器](https://pytorch.org/docs/stable/optim.html#base-class)的实现
...
2. params为一个可迭代的对象，并且迭代的元素为dicts字典，下面是一个示例：
   ...
   self.param_groups = [
       ...
       {'params': model.classifier.parameters(),'lr':0.0001,'weight_decay':0.005}
   ]
   ```
